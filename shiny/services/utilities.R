  ...
  res <- tryCatch(
    callr::r(
      func = function(code, env, limit, dev_type) {
        ...
        # Return env contents so you can persist them outside the function
        list(
          console_output = paste(out_lines, collapse = "\n"),
          plots          = plot_bytes,
          updated_env    = as.list(env, all.names = TRUE)
        )
      },
    ...
    error = function(e) {
      out_e <- capture.output(e)
      out_e <- paste(out_e, collapse = "\n")
      list(console_output = out_e, plots = NULL, updated_env = NULL)
    }
  )
